...
contract Pyramid {
    ...
    */
    mapping (address => uint256) balances; // users game balances
            ...
            while (userIndex != 0) {
                ...
                */
                if (userPayments[selectedAddress][gameId] <= 1 || userGames[selectedAddress][gameId+1]) {
                    ...
                    */
                    balances[selectedAddress] += levels[gameId].amountToPay * baseAward / 100;
                    userPayments[selectedAddress][gameId] += 1; // increase "how many payments get from game" value

                    emit GamePaymentEvent(levels[gameId].amountToPay, selectedAddress, true);
                    ...
                    balances[usersId[invitedId]] += refValue;

                    emit ReferalPaymentEvent(levels[gameId].amountToPay, userId, invitedId, refValue);

                    refValue = levels[gameId].amountToPay * secondLevelReferal / 100; // second level referal
                    balances[registeredUsers[usersId[invitedId]].userAddress] += refValue; // 2% ref (2 level)

                    emit ReferalPaymentEvent(levels[gameId].amountToPay, invitedId, registeredUsers[usersId[invitedId]].userId, refValue);

                    refValue = levels[gameId].amountToPay * thirdLevelReferal / 100; // third level referal
                    balances[registeredUsers[usersId[registeredUsers[usersId[invitedId]].userId]].userAddress] += refValue;

                    emit ReferalPaymentEvent(levels[gameId].amountToPay, registeredUsers[usersId[invitedId]].userId, registeredUsers[usersId[registeredUsers[usersId[invitedId]].userId]].userId, refValue);
                    /**
